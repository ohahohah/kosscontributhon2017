//original code :http://bl.ocks.org/nbremer/326fb6de768e85261bfd47aa1f497863
...
function init() {
    ...
    //main_xScale = d3.scale.linear().range([0, 100]);
    ...
    //main_xScale.domain([0, 100]);
    mini_xScale.domain([0, d3.max(data, function(d) { return d.value; })]);
    main_yScale.domain(data.map(function(d) { return d.date; }));
    mini_yScale.domain(data.map(function(d) { return d.date; }));
    ...
    brush = d3.svg.brush()
        .y(mini_yScale)
        .extent([mini_yScale(data[0].date), mini_yScale(data[brushExtent].date)])
        .on("brush", brushmove)
    ...
    //createGradient("gradient-rainbow-main", "13%");
    ...
    mini_bar
        ...
        .attr("y", function(d, i) { return mini_yScale(d.date); })
...
function update() {
    ...
    //UPDATE
    bar
        .attr("y", function(d, i) { return main_yScale(d.date); })
...
function brushmove() {
    ...
    d3.select(".miniGroup").selectAll(".bar")
        .style("fill", function(d, i) { return selected.indexOf(d.date) > -1 ? "url(#gradient-rainbow-mini)" : "#e0e0e0"; });
    ...
    var newMaxXScale = d3.max(data, function(d) { return selected.indexOf(d.date) > -1 ? d.value : 0; });
...
function scroll() {
    ...
    if (extent[0] - dy < y0) { topSection = y0; } else if (extent[1] - dy > y1) { topSection = y1 - size; } else { topSection = extent[0] - dy; }
...
d3.selectAll("#hBar text").style("fill", "white");
